...
def go_to_point(goal):
    ...
    feedback = summit_xl_action.msg.TargetFeedback()
    result = summit_xl_action.msg.TargetResult()
...
def main():
    ...
    pub_ = rospy.Publisher('robot/cmd_vel', Twist, queue_size=1)
    sub_odom = rospy.Subscriber('robot/odom', Odometry, clbk_odom)
    act_s = actionlib.SimpleActionServer('robot/go_to_point', summit_xl_action.msg.TargetAction, go_to_point, auto_start=False)
    act_s.start()
    rate = rospy.Rate(20)
